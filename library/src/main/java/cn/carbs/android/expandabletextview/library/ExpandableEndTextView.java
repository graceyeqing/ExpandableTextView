...
public class ExpandableEndTextView extends TextView {
    ...
    private int fullLineCount,indexEnd,indexStart;

    public ExpandableEndTextView(Context context) {
        super(context);
        init();
    }

    public ExpandableEndTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init();
    }

    public ExpandableEndTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public CharSequence getNewTextByConfig() {
        ...
        switch (mCurrState) {
            case STATE_SHRINK: {
                ...
                int remainWidth = 0;
//                String subText0 = mOrigText.subSequence(indexStart, indexEnd).toString();
//                if (subText0.contains("\n")) {
//                    remainWidth = (int) (mTextPaint.measureText(mOrigText.subSequence(indexStart, indexEnd - 3).toString()));
//                } else {
                    String subText = mOrigText.subSequence(indexStart, indexEndTrimmed).toString();
                    remainWidth = getValidLayout().getWidth() -
                            (int) (mTextPaint.measureText(subText) + 0.5);
//                }
                ...
                CharSequence fixText = removeEndLineBreak(mOrigText.subSequence(0, indexEnd));
...
//                    drawable.setBounds(new Rect(getRight()-21,-20,getRight(),0));
    ...
    public interface OnExpandListener {
        void onExpand(ExpandableEndTextView view);

        void onShrink(ExpandableEndTextView view);
    }
    ...
    private class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if (hasOnClickListeners()
                    && (getOnClickListener(ExpandableEndTextView.this) instanceof ExpandableClickListener)) {
            } else {
                toggle();
            }
        }
    ...
    public void setFullLineCount(int fullLineCount) {
